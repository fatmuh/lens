    ...
    fn check(&self, file: &FileAnalysis, source: &str) -> Vec<Issue> {
        ...
        crate::analyzer::parser::with_parser(lang, source, |tree| {
            crate::analyzer::parser::visit_descendants(tree.root_node(), |node| {
                ...
                let Ok(c) = consequence.utf8_text(source.as_bytes()) else {
                    return;
                };
                let Ok(a) = alternative.utf8_text(source.as_bytes()) else {
                    return;
                };
                if (c == "true" && a == "false") || (c == "false" && a == "true") {
                    ...
                    issues.push(Issue {
                        ...
                        message: if c == "true" {
                            "Use the condition directly instead of `cond ? true : false`.".into()
                        } else {
                            "Negate the condition directly instead of `cond ? false : true`.".into()
                        },
