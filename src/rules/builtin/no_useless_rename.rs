...
impl Rule for NoUselessRename {
    fn id(&self) -> &'static str { "no-useless-rename" }
    fn name(&self) -> &'static str { "No useless rename" }
    ...
    fn check(&self, file: &FileAnalysis, source: &str) -> Vec<Issue> {
        ...
        crate::analyzer::parser::with_parser(lang, source, |tree| {
            crate::analyzer::parser::visit_descendants(tree.root_node(), |node| {
                if node.kind() != "import_specifier" { return; }
                let Some(name) = node.child_by_field_name("name") else { return; };
                let Some(alias) = node.child_by_field_name("alias") else { return; };
                let Ok(n) = name.utf8_text(source.as_bytes()) else { return; };
                let Ok(a) = alias.utf8_text(source.as_bytes()) else { return; };
                if n == a {
