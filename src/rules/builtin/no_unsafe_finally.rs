...
impl Rule for NoUnsafeFinally {
    fn id(&self) -> &'static str { "no-unsafe-finally" }
    fn name(&self) -> &'static str { "Unsafe control in `finally`" }
    ...
    fn check(&self, file: &FileAnalysis, source: &str) -> Vec<Issue> {
        ...
        crate::analyzer::parser::with_parser(lang, source, |tree| {
            crate::analyzer::parser::visit_descendants(tree.root_node(), |node| {
                if node.kind() != "finally_clause" { return; }
...
fn walk_for_terminators(node: Node, file: &FileAnalysis, source: &str, issues: &mut Vec<Issue>) {
    ...
    for c in node.children(&mut cursor) {
        if matches!(c.kind(), "return_statement" | "throw_statement" | "break_statement" | "continue_statement") {
            let start = c.start_position();
            let end = c.end_position();
            issues.push(Issue {
                rule_id: "no-unsafe-finally".into(),
                severity: Severity::Critical,
                message: format!("`{}` in a `finally` block can swallow exceptions.", c.kind().replace("_statement", "")),
        ...
        if matches!(c.kind(), "statement_block" | "if_statement" | "switch_statement" | "for_statement" | "while_statement" | "try_statement") {
