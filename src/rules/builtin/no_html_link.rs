...
impl Rule for NoHtmlLink {
    fn id(&self) -> &'static str { "no-html-link" }
    fn name(&self) -> &'static str { "No `dangerouslySetInnerHTML`" }
    ...
    fn languages(&self) -> &[Language] { &[Language::Tsx, Language::Jsx] }

    fn check(&self, file: &FileAnalysis, source: &str) -> Vec<Issue> {
        let mut issues = Vec::new();
        let Some(lang) = file.language else { return issues };
        if !matches!(lang, Language::Tsx | Language::Jsx) { return issues; }
...
fn jsx_attribute_name_text(node: Node, source: &str) -> Option<String> {
    ...
    if node.kind() != "jsx_attribute" { return None; }
    ...
    let Ok(text) = node.utf8_text(source.as_bytes()) else { return None; };
