...
impl Rule for PreferPromiseRejectErrors {
    fn id(&self) -> &'static str { "prefer-promise-reject-errors" }
    fn name(&self) -> &'static str { "Reject with `Error`" }
        ...
        crate::analyzer::parser::with_parser(lang, source, |tree| {
            crate::analyzer::parser::visit_descendants(tree.root_node(), |node| {
                ...
                let is_promise_reject = func.kind() == "member_expression"
                    && func.child_by_field_name("object").map(|o| o.utf8_text(source.as_bytes()).unwrap_or("") == "Promise").unwrap_or(false)
                    && func.child_by_field_name("property").map(|p| p.utf8_text(source.as_bytes()).unwrap_or("") == "reject").unwrap_or(false);
                // Case 2: `reject(...)` inside a Promise executor (we
                // can't easily detect this without scope analysis; skip)
                if !is_promise_reject { return; }
                ...
                for arg in node.children(&mut cursor) {
                    if arg.kind() == "arguments" {
                        ...
                        for a in arg.children(&mut ac) {
                            if matches!(a.kind(), "string" | "number" | "true" | "false" | "null" | "undefined" | "template_string") {
