...
impl Rule for NoElseReturn {
    fn id(&self) -> &'static str { "no-else-return" }
    fn name(&self) -> &'static str { "No `else` after `return`" }
    ...
    fn check(&self, file: &FileAnalysis, source: &str) -> Vec<Issue> {
        ...
        crate::analyzer::parser::with_parser(lang, source, |tree| {
            crate::analyzer::parser::visit_descendants(tree.root_node(), |node| {
                ...
                let has_else = node.children(&mut node.walk()).any(|c| c.kind() == "else_clause");
                if !has_else { return; }
                // The consequence must end with a return/throw/break/continue.
                let ends_with_exit = if consequence.kind() == "statement_block" {
                    let mut cursor = consequence.walk();
                    consequence.children(&mut cursor).last().map_or(false, |last| {
                        matches!(last.kind(), "return_statement" | "throw_statement" | "break_statement" | "continue_statement")
                    })
                } else {
                    matches!(consequence.kind(), "return_statement" | "throw_statement" | "break_statement" | "continue_statement")
                };
                if ends_with_exit {
