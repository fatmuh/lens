...
impl Rule for PreferNullishCoalescing {
    fn id(&self) -> &'static str { "prefer-nullish-coalescing" }
    fn name(&self) -> &'static str { "Prefer `??` over `||`" }
            ...
            crate::analyzer::parser::visit_descendants(tree.root_node(), |node| {
                ...
                if is_potentially_nullish(left) && is_potentially_nullish(right) {
                    ...
                    issues.push(Issue {
                        ...
                        message: "Use `??` instead of `||` to allow valid falsy values like `0` or `''`.".into(),
...
fn is_potentially_nullish(node: Node) -> bool {
    matches!(node.kind(), "identifier" | "member_expression" | "call_expression" | "subscript_expression" | "optional_chain")
}
