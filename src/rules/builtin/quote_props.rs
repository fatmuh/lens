...
impl Rule for QuoteProps {
    fn id(&self) -> &'static str { "quote-props" }
    fn name(&self) -> &'static str { "Quote object keys properly" }
    ...
    fn check(&self, file: &FileAnalysis, source: &str) -> Vec<Issue> {
        ...
        crate::analyzer::parser::with_parser(lang, source, |tree| {
            crate::analyzer::parser::visit_descendants(tree.root_node(), |node| {
                ...
                if key.kind() != "property_identifier" { return; }
                let Ok(text) = key.utf8_text(source.as_bytes()) else { return; };
                if needs_quoting(text) {
                    ...
                    issues.push(Issue {
                        ...
                        message: format!("Quote the key `{}` (special characters in identifier).", text),
