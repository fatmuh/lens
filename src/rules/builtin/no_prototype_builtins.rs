...
impl Rule for NoPrototypeBuiltins {
    fn id(&self) -> &'static str { "no-prototype-builtins" }
    fn name(&self) -> &'static str { "No direct prototype builtins" }
    ...
    fn check(&self, file: &FileAnalysis, source: &str) -> Vec<Issue> {
        ...
        crate::analyzer::parser::with_parser(lang, source, |tree| {
            crate::analyzer::parser::visit_descendants(tree.root_node(), |node| {
                ...
                if !FORBIDDEN.contains(&prop_text) { return; }
                let start = node.start_position();
                let end = node.end_position();
                issues.push(Issue {
                    rule_id: "no-prototype-builtins".into(),
                    severity: Severity::Critical,
                    message: format!("Use `Object.prototype.{}.call(...)` instead of direct `{}.`", prop_text, prop_text),
