...
impl Rule for NoParamReassign {
    fn id(&self) -> &'static str { "no-param-reassign" }
    fn name(&self) -> &'static str { "No parameter reassignment" }
    ...
    fn check(&self, file: &FileAnalysis, source: &str) -> Vec<Issue> {
        ...
        crate::analyzer::parser::with_parser(lang, source, |tree| {
            crate::analyzer::parser::visit_descendants(tree.root_node(), |func| {
                ...
                let Some(params) = func.child_by_field_name("parameters") else { return; };
                let mut param_names: HashSet<String> = HashSet::new();
                let mut cursor = params.walk();
                for c in params.children(&mut cursor) {
                    if matches!(c.kind(), "required_parameter" | "optional_parameter" | "identifier") {
                        let name = c.child_by_field_name("pattern")
                            .or_else(|| if c.kind() == "identifier" { Some(c) } else { None });
                        if let Some(n) = name {
                            ...
                            if let Ok(text) = left.utf8_text(source.as_bytes()) {
                                if param_names.iter().any(|p| p == &text) {
                                    ...
                                    issues.push(Issue {
                                        ...
                                        message: format!("Don't reassign parameter `{}`; use a local variable.", text),
                            ...
                            if let Ok(text) = arg.utf8_text(source.as_bytes()) {
                                if param_names.iter().any(|p| p == &text) {
                                    ...
                                    issues.push(Issue {
                                        rule_id: "no-param-reassign".into(),
                                        severity: Severity::Major,
                                        message: format!("Don't mutate parameter `{}`; use a local variable.", text),
                                        file: file.path.clone(),
                                        start_line: start.row as u32 + 1,
                                        end_line: end.row as u32 + 1,
                                        start_column: start.column as u32,
