...
impl Rule for Camelcase {
    fn id(&self) -> &'static str { "camelcase" }
    fn name(&self) -> &'static str { "Use camelCase" }
    ...
    fn check(&self, file: &FileAnalysis, source: &str) -> Vec<Issue> {
        ...
        crate::analyzer::parser::with_parser(lang, source, |tree| {
            crate::analyzer::parser::visit_descendants(tree.root_node(), |node| {
                ...
                let Ok(text) = name.utf8_text(source.as_bytes()) else { return; };
                // Skip already-valid: all lowercase, camelCase, or ALL_CAPS.
                if is_valid(text) { return; }
...
fn is_valid(name: &str) -> bool {
    ...
    if !chars.peek().map_or(false, |c| c.is_ascii_lowercase() || *c == '_') {
        return false;
    }
    // No snake_case.
    if name.contains('_') {
