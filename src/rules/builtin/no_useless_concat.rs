...
impl Rule for NoUselessConcat {
    fn id(&self) -> &'static str { "no-useless-concat" }
    fn name(&self) -> &'static str { "No useless string concatenation" }
    ...
    fn check(&self, file: &FileAnalysis, source: &str) -> Vec<Issue> {
        ...
        crate::analyzer::parser::with_parser(lang, source, |tree| {
            crate::analyzer::parser::visit_descendants(tree.root_node(), |node| {
                ...
                if op.utf8_text(source.as_bytes()).map(|t| t != "+").unwrap_or(true) { return; }
                let Some(left) = node.child_by_field_name("left") else { return; };
                let Some(right) = node.child_by_field_name("right") else { return; };
                if matches!(left.kind(), "string" | "template_string") && matches!(right.kind(), "string" | "template_string") {
                    let start = node.start_position();
                    let end = node.end_position();
                    issues.push(Issue {
                        rule_id: "no-useless-concat".into(),
                        severity: Severity::Minor,
                        message: "Concatenating two literals is unnecessary; use a single literal.".into(),
