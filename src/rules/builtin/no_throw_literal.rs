...
impl Rule for NoThrowLiteral {
    fn id(&self) -> &'static str { "no-throw-literal" }
    fn name(&self) -> &'static str { "No `throw <literal>`" }
    ...
    fn check(&self, file: &FileAnalysis, source: &str) -> Vec<Issue> {
        ...
        crate::analyzer::parser::with_parser(lang, source, |tree| {
            crate::analyzer::parser::visit_descendants(tree.root_node(), |node| {
                ...
                let arg = node.children(&mut node.walk()).find(|c| c.kind() != "throw");
                let Some(arg) = arg else { return; };
                let kind = arg.kind();
                let is_literal = matches!(kind, "string" | "number" | "true" | "false" | "null"
